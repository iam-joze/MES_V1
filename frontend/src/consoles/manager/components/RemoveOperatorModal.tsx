...
export function RemoveOperatorModal({ operator, onConfirm, onCancel }: {
  operator: Operator;
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Failed to remove operator.');
      setIsDeleting(false);
    }
  };
          ...
          <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-2">
            ...
            <div className="space-y-1.5 text-sm text-slate-700">
              <div className="flex items-center gap-2">
                ...
                <span>Account for <strong>{operator.name}</strong> ({operator.phone || operator.identifier}) is deleted</span>
              </div>
              <div className="flex items-center gap-2">
                <Trash2 size={14} className="text-danger-500 flex-shrink-0" strokeWidth={2.5} />
                <span>All login credentials, PIN, and skill certifications removed</span>
              </div>
              <div className="flex items-center gap-2">
                <Trash2 size={14} className="text-danger-500 flex-shrink-0" strokeWidth={2.5} />
                <span>Any job stages assigned to them become <strong>Unassigned</strong></span>
              </div>
            </div>
          </div>
